...
def load_utkface(paths, verbose=-1):
    ...
    for (i, imgpath) in enumerate(paths):
        ...
        try:
            ...
            if age <= 50:
                sensitive_attrs.append(0)  # Bin 0 for age <= 50
            else:
                sensitive_attrs.append(1)  # Bin 1 for age > 50
...
def apply_deepsmote(data, labels, age_bins, args):
    ...
    age_pos = age_bins[pos_indices]
    data_pos = latent_space[pos_indices]
    labels_pos = labels[pos_indices]

    age_groups, counts = np.unique(age_pos, return_counts=True)
    max_count = np.max(counts)

    for age_group in age_groups:
        group_indices = np.where(age_pos == age_group)[0]
        n_current = len(group_indices)
        n_to_sample = max_count - n_current
        if n_to_sample <= 0:
            continue
        X_group = data_pos[group_indices].cpu().numpy()
        samples, _ = G_SM(X_group, labels_pos[group_indices], n_to_sample, pos_class)
        ...
        synthetic_labels.extend([pos_class] * n_to_sample)
...
def reweighted_fedavg(scaled_weights, client_loaders, num_classes=2):
    ...
    for key in scaled_weights[0].keys():
        reweighted_weights[key] = torch.zeros_like(scaled_weights[0][key],dtype=torch.float32)
...
def plot_tsne(original_data, synthetic_data, save_path="tsne_plot_case_1.png"):
    """
    Generate a t-SNE plot for original and synthetic data.
    
    Args:
        original_data: Numpy array of original data (shape: [num_samples, features]).
        synthetic_data: Numpy array of synthetic data (shape: [num_samples, features]).
        save_path: Path to save the t-SNE plot.
    """
    # Combine original and synthetic data
    combined_data = np.vstack([original_data, synthetic_data])
    labels = np.array([0] * len(original_data) + [1] * len(synthetic_data))  # 0 for original, 1 for synthetic

    # Flatten the data if needed (e.g., for images)
    ...
    plt.scatter(tsne_results[labels == 0, 0], tsne_results[labels == 0, 1], label="Original Data", alpha=0.6, s=10)
    plt.scatter(tsne_results[labels == 1, 0], tsne_results[labels == 1, 1], label="Synthetic Data", alpha=0.6, s=10)
...
def main():
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    run_dir = f"run_case12_equatePositiveClass_a{alpha}"
    ...
    plot_tsne(X_server, X_server_smote, save_path="tsne_plot_case_2.png")

    # Train server model on DeepSMOTE-enhanced data
    server_train, server_val, Y_train_srv, Y_val_srv, age_train_srv, age_val_srv = train_test_split(
        X_server_smote, Y_server_smote, age_server_smote, test_size=0.2,
        random_state=22, stratify=Y_server_smote)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("ServerPretrainFairnessMetrics_case_2.png")
    plt.close()
    ...
    with open(os.path.join(run_dir, "entropy_tracking.json"), "w") as f:
        json.dump(to_serializable(entropy_per_round), f, indent=2)
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("global_metrics_case_2.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("global_accuracy_case_2.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("global_loss_case_2.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("client_entropy_case_2.png")  # Save figure
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("client_spd_case_2.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("client_eod_case_2.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("ServerPretrainLoss_case_2.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("ServerPretrainAccuracy_case_2.png")
